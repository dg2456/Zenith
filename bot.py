...
# Verify token exists
if not TOKEN:
    print("[ERROR] DISCORD_TOKEN not found in environment!", file=sys.stderr)
...
class ZenithBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            print(f"[SETUP] Synced {len(synced)} slash commands to guild", file=sys.stderr)
            self.synced = True
        except Exception as e:
...
async def on_ready():
    ...
    print(f"[READY] Commands synced: {bot.synced}", file=sys.stderr)
    cmds = bot.tree.get_commands()
    print(f"[READY] Registered commands: {[c.name for c in cmds]}", file=sys.stderr)
    print("="*60 + "\n", file=sys.stderr)
...
async def promote(interaction: discord.Interaction, user: discord.User, reason: str, new_rank: discord.Role):
    try:
        ...
        try:
            await member.send(f"Congratulations! You have been promoted to {new_rank.name}!\nReason: {reason}")
        except:
            pass
...
async def start_application(interaction: discord.Interaction, app_name: str):
    ...
    embed = discord.Embed(
        ...
        description="This application will expire in 1 hour.",
        color=discord.Color.blue()
    )
...
async def submit_application(user: discord.User, app_id: str):
    ...
    for i, question in enumerate(app_info["questions"]):
        ...
        embed.add_field(name=f"Q{i+1}: {question}", value=response[:1024], inline=False)
...
async def help_command(interaction: discord.Interaction):
    try:
        ...
        embed.set_footer(text="Zenith Bot v1.3.0")
        await interaction.response.send_message(embed=embed, ephemeral=True)
    except Exception as e:
...
async def main():
    print("[MAIN] Starting Zenith Bot...", file=sys.stderr)
    ...
    while True:
        attempt += 1
        print(f"[MAIN] Discord connection attempt #{attempt}...", file=sys.stderr)

        # Create a fresh bot instance each attempt so the closed client isn't reused
        fresh_bot = ZenithBot()

        # Re-register all commands on the fresh instance
        for cmd in list(bot.tree.get_commands()):
            try:
                fresh_bot.tree.add_command(cmd)
            except Exception:
                pass

        try:
            async with fresh_bot:
                await fresh_bot.start(TOKEN)

        except discord.errors.HTTPException as e:
            if e.status == 429:
                retry_after = int(e.response.headers.get('Retry-After', delay))
                print(f"[ERROR] Rate limited by Cloudflare. Waiting {retry_after}s...", file=sys.stderr)
                await asyncio.sleep(retry_after)
                delay = min(delay * 2, 300)
            else:
                print(f"[ERROR] HTTP Exception ({e.status}): {e}", file=sys.stderr)
                traceback.print_exc(file=sys.stderr)
                await asyncio.sleep(delay)
                delay = min(delay * 2, 300)

        except discord.errors.LoginFailure as e:
            print(f"[ERROR] Invalid token - check DISCORD_TOKEN env var: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    print(f"[START] TOKEN present: {'YES' if TOKEN else 'NO - set DISCORD_TOKEN env var!'}", file=sys.stderr)
    print(f"[START] GUILD_ID: {GUILD_ID}", file=sys.stderr)
